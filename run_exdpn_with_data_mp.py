...
parser.add_argument("-n", "--num_models", type=int, default=10)
parser.add_argument("-c", "--num-cores", type=int, default=-1, help="Number of cores to use for multiprocessing. Default: 2 less than the available amount of cores.")
...
def train_model(args) -> BIG_DGCNN:
    ...
    model.train(
        log,
        "expn_p2p_with_origins_and_data_experiment",
        case_level_attributes=["total_price", "item_category", "origin"],
...
def main():
    ...
    log = pm4py.read_xes(r"./Event Logs/eXdpn/exdpn_with_customs.xes")
    print_log_statistics(log)

    out_path = Path("./Experiments/eXdpn/With Customs And Data/")
    ...
    progress = tqdm(total=NUM_MODELS, desc="Training Models")
    models: List[BIG_DGCNN] = [train_model((idx, log)) for idx in range(NUM_MODELS) if progress.update() or True]
    ...
    for idx, model in enumerate(models, start=1):
        model.save(Path(out_path, f"Model_{idx}.pt").as_posix()) # Save the parameters of the internal pytorch model
        # The pickling partly creates files of ~52GB each, so not doing that anymore.
        # with open(f"./Models/Helpdesk/Helpdesk_{idx}.pkl", "wb") as f: # Save the entire BIG_DGCNN object (includes the dataset, etc.) 
        #     pickle.dump(model, f)
        model.plot_training_history().savefig(Path(out_path, f"Model_{idx}.pdf").as_posix())
