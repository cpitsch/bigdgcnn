...
class BIG_Instancegraph_Dataset(InMemoryDataset):
    ...
    def _networkx_to_pytorch_graph(self, graph: nx.DiGraph, label: str) -> Data:
        ...
        x = torch.tensor(np.array([self._make_feature_vector(node) for node in sorted(graph.nodes, key=lambda v: v[0])]), dtype=torch.float)
        data = Data(
            x=x,
            edge_index=edge_index.t().contiguous(),
            y=self._one_hot_encode(label),
            # num_nodes=len(graph.nodes), # To fix a warning (#TODO: Maybe the warning occurs because the graph is structured weirdly?)
        )
        return data     