...
def train_model(args) -> BIG_DGCNN:
    ...
    model.train(log, "helpdesk_repaired")
    return model
...
def main():
    ...
    log = pm4py.read_xes(r"./Event Logs/helpdesk_repaired.xes.gz")
    print_log_statistics(log)

    out_path = Path("./Experiments/Helpdesk Repaired/")
    ...
    for idx, model in enumerate(models, start=1):
        model.save(Path(out_path, f"Helpdesk_Repaired_{idx}.pt").as_posix()) # Save the parameters of the internal pytorch model
        model.plot_training_history().savefig(Path(out_path, f"Helpdesk_Repaired_{idx}.pdf").as_posix())
